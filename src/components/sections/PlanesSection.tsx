...
const defaultPlans: Plan[] = [
  {
    ...
    priority: 'Alta',
    status: 'pendiente',
    notes: 'Reservar mesa con vista al atardecer',
    participants: ['Tú', 'Tu pareja'],
  ...
  },
  {
    ...
    priority: 'Media',
    status: 'pendiente',
    notes: 'Llevar equipo de seguridad y agua',
    participants: ['Tú', 'Tu pareja'],
  ...
  },
  {
    ...
    priority: 'Media',
    status: 'completado',
    notes: 'Incluye 5 restaurantes diferentes',
    participants: ['Tú', 'Tu pareja'],
  ...
  },
  {
    ...
    priority: 'Baja',
    status: 'pendiente',
    notes: 'Entrada gratuita los domingos',
    participants: ['Tú', 'Tu pareja'],
  ...
  },
  {
    ...
    priority: 'Media',
    status: 'en_progreso',
    notes: 'Reservar masaje de pareja',
    participants: ['Tú', 'Tu pareja'],
  ...
  },
  {
    ...
    priority: 'Alta',
    status: 'pendiente',
    notes: 'Llevar mantas y picnic',
    participants: ['Tú', 'Tu pareja', 'Amigos'],
  ...
  },
  {
    ...
    priority: 'Baja',
    status: 'completado',
    notes: 'Comprar ingredientes frescos',
    participants: ['Tú', 'Tu pareja'],
  ...
  },
  {
    ...
    priority: 'Media',
    status: 'pendiente',
    notes: 'Alquilar bicicletas en el centro',
    participants: ['Tú', 'Tu pareja'],
  ...
  },
  {
    ...
    priority: 'Baja',
    status: 'pendiente',
    notes: 'Preparar snacks y bebidas',
    participants: ['Tú', 'Tu pareja'],
  ...
  },
  {
    ...
    priority: 'Alta',
    status: 'pendiente',
    notes: 'Reservar hotel con vista al mar',
    participants: ['Tú', 'Tu pareja'],
...
const categories = [
  { id: 'romantico', label: 'Romántico', icon: Heart, color: 'bg-pink-100 text-pink-800' },
  { id: 'aventura', label: 'Aventura', icon: Mountain, color: 'bg-green-100 text-green-800' },
  { id: 'gastronomico', label: 'Gastronómico', icon: Utensils, color: 'bg-orange-100 text-orange-800' },
  { id: 'cultural', label: 'Cultural', icon: BookOpen, color: 'bg-blue-100 text-blue-800' },
  { id: 'relax', label: 'Relax', icon: Coffee, color: 'bg-purple-100 text-purple-800' },
  { id: 'otro', label: 'Otro', icon: MoreHorizontal, color: 'bg-gray-100 text-gray-800' }
]
...
export function PlanesSection() {
  ...
  const [planForm, setPlanForm] = useState({
    ...
    time: '',
    location: '',
    category: 'romantico' as Plan['category'],
    priority: 'Media' as Plan['priority'],
    ...
    reminderTime: '1h',
    reminderType: 'notification' as 'notification' | 'email' | 'both'
  })
  ...
  const filteredPlans = plans.filter(plan => {
    ...
    const matchesPriority = selectedPriority === 'todos' || plan.priority === selectedPriority
  ...
  const getStats = () => {
    ...
    const withReminders = plans.filter(plan => plan.reminder?.enabled).length

    return { total, pending, inProgress, completed, cancelled, withReminders }
  }
  ...
  const openEditModal = (plan: Plan) => {
    ...
    setPlanForm({
      ...
      reminderTime: plan.reminder?.time || '1h',
      reminderType: plan.reminder?.type || 'notification'
    })
  ...
  const getCategoryColorForCalendar = (category: string) => {
    switch (category) {
      case 'romantico': return 'bg-pink-500'
      case 'aventura': return 'bg-green-500'
      case 'gastronomico': return 'bg-orange-500'
      case 'cultural': return 'bg-blue-500'
      case 'relax': return 'bg-purple-500'
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{stats.withReminders}</p>
              </div>
  ...
  <Progress value={getAchievementProgress()} />
      ...
      {viewMode === 'list' ? (
        ...
        <CalendarView
          ...
          getCategoryColorForCalendar={getCategoryColorForCalendar}
...
interface CalendarViewProps {
  ...
  getCategoryColorForCalendar: (category: string) => string
                  ...
                  <div className="calendar-day-content">
                    ...
                    <div className="space-y-1">
                      {dayPlans.slice(0, 2).map(plan => (
                        <div
                          key={plan.id}
                          className={`h-2 rounded-full calendar-plan-indicator ${getCategoryColorForCalendar(plan.category)}`}
                          title={plan.title}
                        />
                      ))}
                      {dayPlans.length > 2 && (
                ...
                {getPlansForDate(selectedDate).map(plan => (
                  ...
                  >
                    <div className="flex items-center gap-3 flex-1">
                      <div className={`w-3 h-3 rounded-full ${getCategoryColorForCalendar(plan.category)}`} />
                      <div className="flex-1">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <Badge className={getCategoryColor(plan.category)}>
            ...
            {categories.find(c => c.id === plan.category)?.label}
          </Badge>
          <Badge className={getStatusColor(plan.status)}>
        ...
        {plan.reminder?.enabled && (
          <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
            ...
            <span>Recordatorio: {plan.reminder.time} antes</span>
          </div>
        )}
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-2">
          <input
            ...
            className="rounded border-gray-300 text-pink-600 focus:ring-pink-500 dark:border-gray-600 dark:bg-gray-800"
          />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => setForm({ ...form, reminderTime: value })}
              >
                <SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => setForm({ ...form, reminderType: value as 'notification' | 'email' | 'both' })}
              >
                <SelectTrigger>
