...
export function useReminders(plans: Plan[]) {
  ...
  useEffect(() => {
    const checkReminders = () => {
      ...
      plans.forEach(plan => {
        if (!plan.reminder?.enabled || plan.status === 'cancelado') return
        
        const planDate = new Date(plan.date + (plan.time ? `T${plan.time}` : 'T00:00'))
        const reminderTime = parseReminderTime(plan.reminder.time)
        const reminderDate = new Date(planDate.getTime() - reminderTime)
