...
export function useAchievements(plans: Plan[]) {
  ...
  const calculateProgress = (achievement: Achievement, plans: Plan[]) => {
    ...
    const romanticPlans = completedPlans.filter(plan => plan.category === 'romantico')
    const adventurePlans = completedPlans.filter(plan => plan.category === 'aventura')
    const gastronomicPlans = completedPlans.filter(plan => plan.category === 'gastronomico')
    const culturalPlans = completedPlans.filter(plan => plan.category === 'cultural')
    const relaxPlans = completedPlans.filter(plan => plan.category === 'relax')
    const highPriorityPlans = completedPlans.filter(plan => plan.priority === 'Alta')
    const plansWithReminders = plans.filter(plan => plan.reminder?.enabled)
    const plansWithParticipants = plans.filter(plan => plan.participants && plan.participants.length > 0)
    ...
    const differentCategories = new Set(completedPlans.map(plan => plan.category)).size
